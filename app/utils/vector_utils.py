...
def format_vector_for_pg(embedding: np.ndarray) -> str:
    """numpy ベクトルを PostgreSQL vector リテラル文字列に変換"""
    adjusted = adjust_dimension(np.asarray(embedding, dtype=np.float64))
    return "[" + ",".join(map(str, adjusted.tolist())) + "]"
