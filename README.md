...
$dbName = (Get-Content .env | Select-String "POSTGRES_DB" | ForEach-Object { $_.Line.Split('=')[1] }) -replace "dentalDB", "dentalDB"
if (-not $dbName) { $dbName = "dentalDB" }
...
docker exec -e PGPASSWORD=$env:PGPASSWORD postgres_db pg_restore -U $user --clean --if-exists --dbname=dentalDB /tmp/restore.dump
